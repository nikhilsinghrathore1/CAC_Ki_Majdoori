  ...
  return (
    ...
    <div className='w-full h-full absolute z-30 top-0'>
      {/* this is the first container */}
        <div className='w-full border-b border-[1px] pl-5 pt-10 pr-40 tracking-wide border-[#EEE9CC] h-[25vh] flex items-start justify-between'>
        <div onMouseEnter={()=>setconnecting(true)}
             onMouseLeave={()=>setconnecting(false)}
             className='w-1/2 flex items-center text-[11.45rem] mt-4 font-light leading-none f1 h-[30%]  '>
                      <p className=' flex'>
                               c
                               {"onnecting".split("").map((e, i) => (
                              <span className='  w-fit overflow-hidden ' key={i}><motion.span initial={{x:"-100%"}}
        ...
        <div className='w-full border-b border-[1px] pr-5 pl-40  border-[#EEE9CC] h-[25vh] flex items-start justify-between'>
          ...
          <h1 className='capitalize text-[10.5rem] font-light leading-none f1'><span className='-mr-5'>Le</span> Meur</h1>
        </div>
