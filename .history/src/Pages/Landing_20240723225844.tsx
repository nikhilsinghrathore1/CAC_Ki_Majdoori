    ...
    <div className='w-full h-full absolute z-30 top-0'>
      {/* this is the first container */}
        ...
        <div onMouseEnter={()=>setconnecting(true)}
             ...
             className='w-1/2 flex items-center text-[9rem] uppercase mt-10 font-light leading-none f1 h-[30%]  '>
                      <p className=' flex'>
                               c
                               {"onnecting".split("").map((e, i) => (
                              <span className='  w-fit overflow-hidden ' key={i}><motion.span initial={{y:"100%"}}
                              variants={letterVariants}
                              key={i}
                              initial="initial"
                              animate={connecting ? "animate" :"initial"}
                              transition={{ duration: 0.5, delay: i * 0.04, type:"spring" }}          
                               className='block -translate-x-10'>{e}</motion.span></span>
                         ))}
                      </p>
        </div>
