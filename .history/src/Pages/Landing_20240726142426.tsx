import { useGSAP } from '@gsap/react';
import { delay, motion } from 'framer-motion'
import gsap ,{Power4, ScrollTrigger} from 'gsap/all';
import React, { useEffect, useRef, useState } from 'react'
import Marquee from '../components/Marquee';
import SecondTest from './SecondTest';

const image =[
  {
   img: "https://cacclub.in/assets/img/SLider/lop.jpeg",
   first:"50% 50%",
   second:"50% 107.9%",
   third:"50% 165%",
   fourth:"50% 222%"
  },
  {
   img: "https://cacclub.in/assets/img/SLider/Slider%202.png",
   first:"50% 0%",
   second:"50% 400%",
   third:"50% 800%",
   fourth:"50% 1200%"
  }
]





const letterVariants = {
  initial: { y: -50, opacity: 0 },
  animate: { y: 0, opacity: 1 },
}
const letterVariants2 = {
  initial: { x: -50, opacity: 0 },
  animate: { x: 0, opacity: 1 },
};
const letterVariants3 = {
  initial: { y: 50, opacity: 0 },
  animate: { y: 0, opacity: 1 , delay:4 },
};





const Landing = () => {

let clip = 100


  const clipRef = useRef(null) sensing layer analysis of transimitted data  application : rfid  tricopter hybrid vehicals autonomus mapping navigation 
  const topRef = useRef(null)
  const behindRef = useRef(null)
  const firstRef = useRef(null)
  const secondRef = useRef(null)
  const thirdRef = useRef(null)
  const fourthRef = useRef(null)
  const opacTextRef= useRef(null)
  
  const overlayRefs = useRef<(HTMLDivElement | null)[]>([]);
  const testRef = useRef<(HTMLDivElement | null)[]>([]);


  const setOverlayRef = (index: number) => (el: HTMLDivElement | null) => {
    overlayRefs.current[index] = el;
  };


  const [img, setimg] = useState(image[1])


  const [connecting, setconnecting] = useState(false)


  const [all, setAll] = useState(false);


  const [circle, setCircle] = useState(false);




useEffect(() => {
  
  gsap.registerPlugin(ScrollTrigger)
  


    const tl = gsap.timeline()
    const tl2 = gsap.timeline(
      {
        scrollTrigger:{
          trigger:topRef.current,
          start:"top top", 
          end:"bottom top",
          scrub:5,
  
          pin:true,
          // markers:true,
        },
        
      }
    )
    gsap.set(behindRef.current,{scale:3})

    tl2.to(clipRef.current,{
     
      clipPath: "circle(0% at 50% 50%)",
      duration:2
      
      
    },"a")
    tl2.to(behindRef.current,{
      scale:1,
      duration:2
     
    
    },"a")
    tl2.to(firstRef.current,{
      x:10,
      duration:4
    },"a")
    tl2.to(secondRef.current,{
      x:10,
      duration:4
    },"a")
    tl2.to(thirdRef.current,{
      x:10,
      duration:4
    },"a")
    tl2.to(fourthRef.current,{
      x:10,
      duration:4
    },"a")
    tl2.to(opacTextRef.current,{
      delay:2,
      opacity:1,
      duration:2
    },"a")
    tl2.to(behindRef.current,{
      delay:2
    })
    

    const overlayEffect = ()=>{
     
      tl.to(overlayRefs.current, {
        stagger: 0.15,
        top: "0%",
        onComplete: () => {
          setimg((prev) => (prev === image[1] ? image[0] : image[1]));
          
        }
      });
  
      tl.to(overlayRefs.current, {
        delay: 1,
        stagger: 0.25,
        top: "-100%",
      });

   

 
      
     
    }

    

    setInterval(overlayEffect,10000)
  }, [])



  return (
    <div ref={topRef} className='w-full relative  text-[#EEE9CC] h-[110vh] bg-[#1E1915]'>
      {/* main div to be clipped  */}
      <div ref={clipRef} style={{clipPath:`circle(${clip+"%"} at 50% 50%)`}} className={`w-full ${"--clip:0%"} h-full bg-black z-10 clipped absolute top-0`}>

      {/* this is the backgraund */}
      <div className='w-full h-full '>
        {/* first image */}
        <div className='w-full h-[23%] overflow-hidden relative peer-[hover]:scale-150 bg-red-50'>
         

          <img  className='w-full h-[110vh] scale-100 object-cover origin-top  trsni' style={{objectPosition:img.first}} src={img.img} alt="not showing" />
    
          <div className='w-full h-full bg-black/30 absolute top-0' ></div>
          {/* <div className='w-full h-full bg-black absolute top-[0%] '></div> */}
        </div>
        {/* second image  */}
        <div className='w-full h-[22.5%] relative overflow-hidden'>
         

          <img  className='w-full h-[110vh]  origin-center transi object-cover' style={{objectPosition:img.second}} src={img.img} alt="not showing" />

          <div className='w-full h-full bg-black/30 absolute top-[0%] '></div>
        </div>

        <div className='w-full h-[22.9%] relative overflow-hidden'>

          <img  className='w-full h-[110vh] transi object-cover' style={{objectPosition:img.third}} src={img.img} alt="not showing" />
          <div className='w-full h-full absolute bg-black/30 top-[0%] '></div>
        </div>

        <div className='w-full h-[22.7%] relative overflow-hidden'>

          <img  className='w-full h-[110vh] transi origin-bottom scale-100 object-cover' style={{objectPosition:img.fourth}} src={img.img} alt="not showing" />
          <div className='w-full h-full bg-black/30 absolute top-[0%] '></div>
        </div>
        

      </div>

    {/* this is the text portion  */}

    <div className='w-full h-full absolute z-30 top-0'>
      {/* this is the first container */}
        <div
         onMouseEnter={()=>setconnecting(true)}
         onMouseLeave={()=> setTimeout(() => {
          setconnecting(false) 
         }, 2000)}
        className='w-full peer overflow-hidden border-b relative border-[1px] pl-5   pr-40 tracking-wide border-[#EEE9CC] h-[25vh] flex items-start justify-between'>
        <div className='w-1/2  relative  flex items-center text-[#e3f3ff] text-[10rem] z-20  mt-[53px] font-light leading-none f1 h-[30%]  '>
                      <p className=' flex'>
                        C
                               {"onnecting".split("").map((e, i) => (
                              <span className='w-fit  overflow-hidden ' key={i}><motion.span 
                              variants={letterVariants}
                              key={i}
                              initial="initial"
                              animate={connecting ? "animate" :"initial"}
                              transition={{ duration: 0.5, delay: i * 0.04, type:"spring" }}          
                               className='block py-2 '>{e}</motion.span></span>
                         ))}
                      </p>
        </div>
          <p className='text-[1.45rem] mt-4 font-light leading-none f1'>Design de niko</p>

          <div ref={setOverlayRef(0)} className='absolute z-10 top-[100%] bg-black  h-full w-full left-0'/>

        </div>

        {/* second container */}
        <div   
               onMouseEnter={() => setAll(true)}
               onMouseLeave={()=> setTimeout(() => {
                setAll(false) 
               }, 2000)}
           className='w-full f1 relative overflow-hidden border-b border-[1px] pr-5 pl-40  border-[#EEE9CC] h-[25vh] flex items-start justify-between'>
          <p className='text-[1.45rem] mt-4 font-light leading-none f1'>Design d'objet</p>
          <div
        
          className="w-1/2 text-[#FFB347] text-[11rem] pt-20 relative z-20 flex justify-start pl-20 items-center h-[30%]"
        >
          <p className="flex items-center">
                                A
            {"ll".split("").map((e, i) => (
              <span className=" mr-4 w-fit overflow-hidden" key={i}>
                <motion.span
                  variants={letterVariants2}
                  initial="initial"
                  animate={all ? "animate" : "initial"}
                  transition={{ duration: 0.3, delay: i * 0.15  }}
                  className="block"
                >
                  {e}
                </motion.span>
              </span>
            ))}
          </p>
        </div>

        <div ref={setOverlayRef(1)} className='absolute z-10 top-[100%] bg-black h-full w-full left-0'/>


        </div>

        {/* third container */}

        <div
        onMouseEnter={() => setCircle(true)}
        onMouseLeave={()=> setTimeout(() => {
          setCircle(false) 
         }, 2000)}
         className='w-full border-b border-[1px] overflow-hidden pl-5 pr-[440px] relative  border-[#EEE9CC] h-[25vh] flex items-start justify-between'>
        <div
          className="w-1/2 f1 relative z-20 text-[#FF6347] flex pt-20 text-[10rem] items-center h-[30%]"
        >
          <p className="flex">
            C
            {"ircle".split("").map((e, i) => (
              <span className=" mr-4 w-fit overflow-hidden" key={i}>
                <motion.span
                  variants={letterVariants3}
                  initial="initial"
                  animate={circle ? "animate" : "initial"}
                  transition={{ duration: 0.5, delay: i * 0.04 , type:"spring" }}
                  className="block"
                >
                  {e}
                </motion.span>
              </span>
            ))}
          </p>
        </div>
          <p className='text-[1.45rem] mt-4 font-light leading-none f1'>Design d'évènement</p>
          <div ref={setOverlayRef(2)}  className='absolute z-10 top-[100%] bg-black h-full w-full left-0'></div>

        </div>

        {/* fourth container */}

        <div className='w-full border-b overflow-hidden border-[1px] pr-5 pl-8  border-[#EEE9CC] h-[25vh] flex items-start justify-between relative'>
          <p className='text-[1.45rem] mt-4 font-light leading-none f1'>Design web</p>

          <h1 className='capitalize relative z-20 text-[10.5rem] font-light leading-none f1'>Graphiste</h1>
              <div ref={setOverlayRef(3)} className='absolute z-10 top-[100%] bg-black h-full w-full left-0'/>
        </div>
    </div>
    </div>




    <div className='w-full  h-screen bg-[#AEDEE0] relative   '>

<div ref={opacTextRef} className='w-full opacity-0 text-center f2 font-semibold text-[20.5px] leading-[10px] pt-[90px] tracking-wide'>
  <h1 className='mb-4'>We foster growth through success</h1>
  <h1>that is more than just a club:</h1>
</div>                                    

<div ref={behindRef} className='absolute top-[55%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-full whitespace-nowrap overflow-hidden '>
  <div ref={firstRef} className="translate-x-[40%]">

<Marquee text={["Competition" ,"Hackathon","Achievement" , "nikoChan"]} dist={20}/>
  </div>
  <div ref={secondRef} className="translate-x-[-40%]">

<Marquee text={["Competition" ,"Hackathon","Achievement" , "nikoChan"]} dist={40}/>
  </div>

  <div ref={thirdRef} className='translate-x-[40%]'>
<Marquee text={["Competition" ,"Hackathon","Achievement" , "nikoChan"]} dist={30}/>
  </div>

  <div ref={fourthRef} className="translate-x-[-40%]">

<Marquee text={["Competition" ,"Hackathon","Achievement" , "nikoChan"]} dist={80}/>

  </div>

</div>                                                               
</div>


    </div>
  )
}

export default Landing