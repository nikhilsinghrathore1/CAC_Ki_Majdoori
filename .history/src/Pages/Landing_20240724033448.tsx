...
const Landing = () => {
  ...
  const imgScaleRefs = useRef<(HTMLDivElement | null)[]>([]);
  ...
  useGSAP(()=>{
    ...
    const overlayEffect = ()=>{
      ...
      // Enlarge images
      tl.to(imgScaleRefs.current, {
        duration:0,
        y: "200%",
        ease: "power1.out"
      }, "a");
  
      // Scale images back to normal size
      tl.to(imgScaleRefs.current, {
        delay: 0.5,  // Ensure there's enough time for the enlarge effect to be visible
        stagger: 0.1,
        Y: "100%",
        ease: "power1.in"
      }, );
    ...
    setInterval(overlayEffect,5000)
  })
  ...
  return (
    <div className='w-full relative text-[#EEE9CC] h-[110vh] bg-[#1E1915]'>
      ...
      <div className='w-full h-full'>
        ...
        <div className='w-full h-[23%] overflow-hidden relative peer-[hover]:scale-150 bg-red-50'>
          <div ref={testRef} className=' origin-top'>

          <img ref={setimgRef(0)} className='w-full h-[110vh] scale-100 object-cover origin-top  trsni' style={{objectPosition:img.first}} src={img.img} alt="not showing" />
          </div>
        ...
        <div className='w-full h-[22.5%] relative overflow-hidden'>
          <img ref={setimgRef(1)} className='w-full h-[110vh]  origin-center transi object-cover' style={{objectPosition:img.second}} src={img.img} alt="not showing" />
          <div className='w-full h-full bg-black/30 absolute top-[0%] '></div>
        </div>

        <div className='w-full h-[22.9%] relative overflow-hidden'>
          <img ref={setimgRef(2)} className='w-full h-[110vh] transi object-cover' style={{objectPosition:img.third}} src={img.img} alt="not showing" />
          <div className='w-full h-full absolute bg-black/30 top-[0%] '></div>
        </div>

        <div className='w-full h-[22.7%] relative overflow-hidden'>
          <img ref={setimgRef(3)} className='w-full h-[110vh] transi origin-bottom scale-100 object-cover' style={{objectPosition:img.fourth}} src={img.img} alt="not showing" />
          <div className='w-full h-full bg-black/30 absolute top-[0%] '></div>
        </div>
    ...
    <div className='w-full h-full absolute z-30 top-0'>
      {/* this is the first container */}
        ...
        <div className='w-1/2  relative z-20 flex items-center text-[#e3f3ff] text-[10rem]  mt-[53px] font-light leading-none f1 h-[30%]  '>
                      <p className=' flex'>
                        C
